...
var HTTPReaderStream = function(options) {
	...
	var parseHeaders = function() {
		...
		headers.forEach(function(line) {
			...
			if (match) {
				...
				currentHeaders.push([match[1], match[2]]);
			}
		});
		self.emit('headers', currentHeaders);
		handleState();
	};
	...
	var parseContent = function() {
		var cntlen = currentHeaders.filter(function(item) {
			return item[0] === 'content-length';
		}).shift();
		var len = cntlen ? parseInt(cntlen[1].trim()) : data.length;
		if (len > data.length) {
