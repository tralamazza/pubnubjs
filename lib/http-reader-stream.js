...
var HTTPReaderStream = function(options) {
	...
	var parseStatusLine = function() {
		...
		var statusLine = consts.HTTP_StatusLine.exec(data.slice(0, p));
		if (!statusLine) {
			return;
		}
		data = data.slice(p + 2);
	...
	var parseHeaders = function() {
		...
		headers.forEach(function(line) {
			var match = consts.HTTP_Header.exec(line.toLowerCase());
			if (match) {
				if (match[1] === 'transfer-encoding' && match[2].trim() === 'chunked') {
					state = States.TRY_CHUNK;
				}
				parsedHeaders.push([match[1], match[2]]);
			}
		});
		self.emit('headers', parsedHeaders);
		handleState();
	};
	...
	var parseChunk = function() {
		...
		self.push(tmp.slice(0, size + 1));
		data = tmp.slice(size + 2);
		parseChunk(); // XXX convert this to a proper loop
	};

	var parseContent = function() {
		var cntlen = parsedHeaders.filter(function(item) {
			return item[0] === 'content-length';
		}).shift();
		var len = cntlen ? data.length : parseInt(cntlen[1].trim());
		if (len > data.length) {
			// XXX we should have a limit here, or we could buffer "forever"
			return;
		}
		self.push(data.slice(0, len + 1));
